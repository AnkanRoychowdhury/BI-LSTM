...
y_val = encoder.transform(df_val['family_name'].values)
...
X_val = X_val.reshape((X_val.shape[0], n_timesteps, X_val.shape[1]))
...
history = model.fit(X_train, y_train, batch_size=256, epochs=500, verbose=1,callbacks=[early_stopping])
...
report = classification_report(y_test, y_pred, labels=np.unique(y_test), target_names=encoder.classes_)
